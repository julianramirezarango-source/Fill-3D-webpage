...
# ── Step 1: Add upstream remote (OrcaSlicer/OrcaSlicer) ──────────────────────
print("Step 1: Setting up upstream remote...")
remotes = run(["git", "remote"]).stdout
if "upstream" not in remotes:
    r = run(["git", "remote", "add", "upstream", "https://github.com/OrcaSlicer/OrcaSlicer.git"])
else:
    run(["git", "remote", "set-url", "upstream", "https://github.com/OrcaSlicer/OrcaSlicer.git"])

# ── Step 2: Fetch latest OrcaSlicer main ─────────────────────────────────────
print("\nStep 2: Fetching OrcaSlicer/OrcaSlicer main (this may take a minute)...")
r = run(["git", "fetch", "upstream", "main"])
if r.returncode != 0:
    print("ERROR: Could not fetch from OrcaSlicer. Check your internet connection.")
    sys.exit(1)

# ── Step 3: Reset branch to upstream/main ────────────────────────────────────
print("\nStep 3: Resetting branch to upstream/main...")
r = run(["git", "reset", "--hard", "upstream/main"])
if r.returncode != 0:
    print("ERROR: git reset failed.")
    sys.exit(1)

# ── Step 4: Write the 12 new profile files (NOT PLA Turbo) ───────────────────
print("\nStep 4: Writing FILL3D profile files...")
...
# ── Step 5: Update OrcaFilamentLibrary.json ───────────────────────────────────
print("\nStep 5: Updating OrcaFilamentLibrary.json...")
...
for filename in FILES:
    ...
    if name not in existing:
        new_entries.append({"name": name, "sub_path": f"filament/FILL3D/{filename}"})
...
r = run(["git", "commit", "-m",
         "Add FILL3D filament profiles: PLA Basic, PETG, PETG CF, PP, PPCF, PA"])
if r.returncode != 0:
    print("Nothing new to commit — branch may already be up to date.")

# ── Step 7: Force-push ────────────────────────────────────────────────────────
print("\nStep 7: Force-pushing to GitHub...")
r = run(["git", "push", "--force-with-lease", "origin", "add-fill3d-additional-profiles"])
if r.returncode != 0:
    print("\nERROR: Push failed. Try running manually:")
    print("  git push --force origin add-fill3d-additional-profiles")
    sys.exit(1)

print("\nDone! Your PR conflicts should now be resolved.")
print("Check: https://github.com/julianramirezarango-source/OrcaSlicer/compare/add-fill3d-additional-profiles")
