...
function SidebarItem({
  ...
  const iconColor = item.icon === 'triangle' ? 'text-amber-500' : 'text-[#5E33D9]'
    ...
    <li>
      <button
        ...
        className={`w-full flex items-center gap-2.5 px-3 py-2 text-sm rounded-lg transition-colors cursor-pointer
          ${isActive
            ? 'bg-[#F3EEFF] text-[#5E33D9] font-medium'
            : 'text-gray-500 hover:text-gray-800 hover:bg-gray-50'}`}
      >
        <NavIcon type={item.icon} className={isActive ? 'text-[#5E33D9]' : iconColor} />
...
function WikiHome({
  ...
  const CARD_ICONS: Record<string, React.ReactNode> = {
    'Lo básico': (
      <svg className="w-10 h-10 text-[#5E33D9]" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
    ...
    <div className="max-w-3xl">
      ...
      <div className="mb-8">
        <div className="flex items-center gap-2 mb-3">
          <svg className="w-5 h-5 text-[#5E33D9]" fill="currentColor" viewBox="0 0 24 24">
            <path d="M11.645 20.91l-.007-.003-.022-.012a15.247 15.247 0 01-.383-.218 25.18 25.18 0 01-4.244-3.17C4.688 15.36 2.25 12.174 2.25 8.25 2.25 5.322 4.714 3 7.688 3A5.5 5.5 0 0112 5.052 5.5 5.5 0 0116.313 3c2.973 0 5.437 2.322 5.437 5.25 0 3.925-2.438 7.111-4.739 9.256a25.175 25.175 0 01-4.244 3.17 15.247 15.247 0 01-.383.219l-.022.012-.007.004-.003.001a.752.752 0 01-.704 0l-.003-.001z" />
          </svg>
          <span className="text-sm font-semibold text-[#5E33D9]">Wiki Fill-3D</span>
        </div>
      ...
      <section className="mb-8">
        <div className="flex items-center gap-2.5 mb-4">
          <svg className="w-4 h-4 text-[#5E33D9]" viewBox="0 0 16 16" fill="currentColor">
      ...
      <section>
        <div className="flex items-center gap-2.5 mb-5">
          <svg className="w-4 h-4 text-[#5E33D9]" fill="currentColor" viewBox="0 0 24 24">
          ...
          {nav.map(group => {
            ...
            return (
              <button
                ...
                className="text-left bg-[#F3EEFF] hover:bg-[#E4D5FF] border border-[#E4D5FF] hover:border-[#C8AAFF] rounded-2xl p-5 transition-all cursor-pointer group"
              >
                <div className="mb-4">{CARD_ICONS[group.label] ?? <NavIcon type="circle" className="w-10 h-10 text-[#5E33D9]" />}</div>
                <h3 className="font-semibold text-gray-800 text-sm mb-1 group-hover:text-[#5E33D9] transition-colors">
                  {group.label}
                </h3>
...
export default function WikiClient() {
  ...
  useEffect(() => {
    if (!currentPath) { setHtml(''); setTitle(''); return }
    setLoading(true)
  ...
  return (
    <div className="min-h-screen bg-white flex flex-col">
      ...
      <header className="sticky top-0 z-50 bg-white border-b border-gray-100 h-[60px] flex items-center px-5 gap-4">
        ...
        >
          <img src="/wiki/logo.svg" alt="Fill-3D" className="h-7 w-auto" />
          <span className="font-bold text-gray-900 text-lg tracking-tight">Fill-3D</span>
          <span className="text-[9px] font-bold bg-[#5E33D9] text-white px-1.5 py-[3px] rounded uppercase tracking-widest">
            Wiki
          </span>
        </button>
        ...
        <nav className="flex items-center gap-1">
          ...
          <a
            ...
            target="_blank"
            rel="noopener noreferrer"
            className="text-sm font-semibold text-white bg-[#5E33D9] hover:bg-[#4A28AA] px-4 py-1.5 rounded-xl transition-colors ml-1"
          >
            Pedir →
          </a>
        </nav>
            ...
            <article className="max-w-3xl">
              ...
              {bc && (
                <nav className="flex items-center flex-wrap gap-1 text-xs text-gray-400 mb-6">
                  <button
                    ...
                    className="hover:text-[#5E33D9] transition-colors cursor-pointer"
                  >
                    Inicio
                  </button>
                  <span>›</span>
              ...
              <div className="mt-12 pt-6 border-t border-gray-100 flex flex-wrap items-center justify-between gap-3 text-xs text-gray-400">
                ...
                <a
                  ...
                  target="_blank"
                  rel="noopener noreferrer"
                  className="text-[#5E33D9] hover:text-[#4A28AA] font-medium transition-colors"
                >
                  ¿Dudas? WhatsApp →
                </a>
              </div>
